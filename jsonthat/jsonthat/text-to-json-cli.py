...
def get_provider(config: Config) -> LLMProvider:
    provider_name = os.environ.get('LLM_PROVIDER') or config.get('provider', 'openai').lower()
    api_key = os.environ.get('LLM_API_KEY') or config.get('api_key')

    if not api_key:
        raise ValueError("API key not found. Please run the setup command or set the LLM_API_KEY environment variable.")
